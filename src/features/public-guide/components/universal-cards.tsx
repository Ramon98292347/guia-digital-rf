...
export function AccommodationCard({ item, onClick }: { item: PublicGuideAccommodation; onClick: () => void }) {
  return <GuideCard onClick={onClick}><div className="overflow-hidden rounded-2xl" aria-label={`Ver detalhes de ${item.name}`}><div className="relative aspect-[4/3] bg-[var(--guide-muted-bg)]">{item.imageUrl ? <img src={item.imageUrl} alt={item.name} loading="lazy" className="h-full w-full object-cover" /> : <ImageIcon className="absolute inset-0 m-auto size-7 text-[var(--guide-primary)]" aria-hidden="true" />}</div><div className="p-3"><h3 className="font-semibold text-[var(--guide-foreground)]">{item.name}</h3>{item.short_description ? <p className="mt-1 line-clamp-2 text-xs text-[var(--guide-muted)]">{item.short_description}</p> : null}{item.capacity ? <p className="mt-2 text-xs text-[var(--guide-muted)]">Capacidade: {item.capacity}</p> : null}<span className="mt-3 inline-flex items-center gap-1 text-xs font-semibold text-[var(--guide-primary)]">Ver detalhes <ChevronRight className="size-3.5" /></span></div></div></GuideCard>;
}

export function VideoCard({ media, onClick }: { media: PublicGuideMedia; onClick: () => void }) {
  return <GuideCard onClick={onClick}><div className="flex items-center gap-3 p-3"><div className="relative flex size-16 shrink-0 items-center justify-center overflow-hidden rounded-xl bg-[var(--guide-muted-bg)]">{media.url ? <video src={media.url} playsInline preload="metadata" className="h-full w-full object-cover" /> : null}<PlayCircle className="absolute size-7 text-white drop-shadow" /></div><div className="min-w-0"><p className="truncate font-medium text-[var(--guide-foreground)]">{media.caption ?? "Vídeo informativo"}</p><span className="mt-1 inline-flex items-center gap-1 text-xs font-semibold text-[var(--guide-primary)]">Ver vídeo <ChevronRight className="size-3.5" /></span></div></div></GuideCard>;
}
