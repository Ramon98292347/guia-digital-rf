...
export async function getUniversalContentAdminData(tenantSlug: string) {
  ...
  const [collections, items, accommodations, media] = await Promise.all([
  ...
  return { context, collections: ((collections as { data?: unknown }).data ?? []) as Record<string, unknown>[], items: ((items as { data?: unknown }).data ?? []) as Record<string, unknown>[], accommodations: accommodations.data, media: media.data };
}
